...
import umi, {RequestOptionsInit} from "umi-request"
...
export let transcoding = function (host: string, metadata ?: { [k: string]: string }, interceptor?: (resp: Response) => Promise<void>): IRpcImpl {
    ...
    return async function (desc: Method, message: Message | { [k: string]: any }, meta?: { [k: string]: string }): Promise<Message> {
        ...
        const request: RequestOptionsInit = {
        ...
        if (request.data) {
            (<any>request.headers)["Content-Type"] = "application/x-protobuf"
            request.data = request.data.slice(0, request.data.length).buffer
        }

        let response = await umi(host + url, request)
        if (interceptor) {
